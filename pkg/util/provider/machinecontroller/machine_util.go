...
func (c *controller) updateMachineStatusAndNodeCondition(ctx context.Context, machine *v1alpha1.Machine, description string, state v1alpha1.MachineState, drainError error) (machineutils.RetryPeriod, error) {
	if drainError != nil {
		updateRetryPeriod, updateErr := c.updateMachineStatusForDrain(ctx, machine, description, state, v1alpha1.MachineOperationDrainNode)
		if updateErr != nil {
...
func (c *controller) updateMachineStatusForDrain(ctx context.Context, machine *v1alpha1.Machine, description string, state v1alpha1.MachineState, opType v1alpha1.MachineOperationType) (machineutils.RetryPeriod, error) {
	retryPeriod, err := c.machineStatusUpdate(
		ctx,
		machine,
		v1alpha1.LastOperation{
			Description:    description,
...
// getEffectiveMachinePreserveTimeout returns the MachinePreserveTimeout set on the machine-object, otherwise returns the timeout set using the global-flag.
func (c *controller) getEffectiveMachinePreserveTimeout(machine *v1alpha1.Machine) *metav1.Duration {
	var effectivePreserveTimeout *metav1.Duration
	if machine.Spec.MachineConfiguration != nil && machine.Spec.MachineConfiguration.MachinePreserveTimeout != nil {
		effectivePreserveTimeout = machine.Spec.MachineConfiguration.MachinePreserveTimeout
	} else {
		effectivePreserveTimeout = &c.safetyOptions.MachinePreserveTimeout
	}
	return effectivePreserveTimeout
}
...
func (c *controller) UpdateNodeTerminationCondition(ctx context.Context, machine *v1alpha1.Machine) error {
	...
	_, err = nodeops.AddOrUpdateConditionsOnNode(ctx, c.targetCoreClient, nodeName, terminationCondition)
	if apierrors.IsNotFound(err) {
		return nil
	}
	return err
...
func (c *controller) setPreserveExpiryTimeOnMachine(ctx context.Context, machine *v1alpha1.Machine) (*v1alpha1.Machine, error) {
...
func (c *controller) clearMachinePreserveExpiryTime(ctx context.Context, machine *v1alpha1.Machine) (*v1alpha1.Machine, error) {
	if machine.Status.CurrentStatus.PreserveExpiryTime == nil {
		return machine, nil
	}
	machine.Status.CurrentStatus.PreserveExpiryTime = nil
